...
--data-urlencode 'query=avg(rate(http_requests_received_total{action=~"",job!="mixvel-ets.*"}[36h])/129600) by(job)' | jq '' > $fhome"rps1.txt"



cat $fhome"rps1.txt" | jq '.data.result[].metric.job' | sed 's/\"/ /g' | sed 's/-api-metrics/ /g' | sed 's/mixvel-/ /g' | sed 's/^[ \t]*//;s/[ \t]*$//' > $fhome"rps2.txt"
cat $fhome"rps1.txt" | jq '.data.result[].value[1]' | sed 's/\"/ /g' | sed 's/^[ \t]*//;s/[ \t]*$//' | awk '{printf("%.8f \n",$1)}' > $fhome"rps3.txt"
